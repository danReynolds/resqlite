...
void main() {
  group('Database', () {
    ...
    test('selectBytes encodes real integer-valued numbers', () async {
      ...
      for (var i = 0; i < values.length; i++) {
        await db.execute('INSERT INTO t(id, v) VALUES (?, CAST(? AS REAL))', [
          i + 1,
          values[i],
        ]);
      }
    ...
    test(
      'six-column executeBatch threshold preserves text and blobs',
      () async {
        await db.execute(
          'CREATE TABLE t('
          'id INTEGER PRIMARY KEY, '
    ...
    test('wide executeBatch reuses repeated large blob parameters', () async {
      await db.execute(
        'CREATE TABLE t('
        'id INTEGER PRIMARY KEY, '
        'b0 BLOB NOT NULL, b1 BLOB NOT NULL, '
        'b2 BLOB NOT NULL, b3 BLOB NOT NULL, '
        'b4 BLOB NOT NULL, b5 BLOB NOT NULL, '
        'b6 BLOB NOT NULL, b7 BLOB NOT NULL'
        ')',
      );

      final payload = Uint8List.fromList(
        List.generate(1024, (i) => (i * 17 + 0xa5) & 0xff),
      );
      final List<List<Object?>> paramSets = [
      ...
      await db.executeBatch(
        'INSERT INTO t(b0, b1, b2, b3, b4, b5, b6, b7) '
        'VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        paramSets,
      );

      final count = await db.select('SELECT COUNT(*) AS cnt FROM t');
      expect(count.single['cnt'], 1000);

      final rows = await db.select(
        'SELECT b0, b3, b7 FROM t WHERE id IN (1, 500, 1000) ORDER BY id',
      );
      expect(rows, hasLength(3));
      for (final row in rows) {
        expect(row['b0'], payload);
        expect(row['b3'], payload);
        expect(row['b7'], payload);
      }
    });
