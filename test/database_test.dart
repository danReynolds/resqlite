...
void main() {
  group('Database', () {
    ...
    test(
      'select preserves dynamic types across numeric batch boundaries',
      () async {
        ...
        void expectValues(List<Map<String, Object?>> rows) {
          expect(rows, hasLength(values.length));
        ...
        expectValues(
          await db.select('SELECT value FROM dynamic_values ORDER BY rowid'),
        );
        await db.transaction((tx) async {
          expectValues(
            await tx.select('SELECT value FROM dynamic_values ORDER BY rowid'),
          );
        });
      },
    );
