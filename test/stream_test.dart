...
void main() {
  group('Database.stream', () {
    ...
    test(
      'batched re-query error does not block successful peer stream',
      () async {
        await db.execute('INSERT INTO items(name, value) VALUES (?, ?)', [
          'alice',
          1,
        ...
        final brokenInitial = Completer<void>();
        final healthyInitial = Completer<void>();
        final brokenError = Completer<Object>();
        final healthyUpdated = Completer<void>();
        final healthyRows = <List<Map<String, Object?>>>[];

        final brokenSub = db
            .stream('SELECT name, value FROM items WHERE id = 1')
            .listen(
              (_) {
                if (!brokenInitial.isCompleted) brokenInitial.complete();
              },
              onError: (error, stackTrace) {
                if (!brokenError.isCompleted) brokenError.complete(error);
              },
            );
        final healthySub = db
            .stream('SELECT name FROM items WHERE id = 1')
            .listen((rows) {
              healthyRows.add(rows);
              if (!healthyInitial.isCompleted) {
                healthyInitial.complete();
                return;
              }
              if (rows.first['name'] == 'alice!' &&
                  !healthyUpdated.isCompleted) {
                healthyUpdated.complete();
              }
            });

        await Future.wait([
          brokenInitial.future,
          healthyInitial.future,
        ]).timeout(const Duration(seconds: 2));

        await db.transaction((tx) async {
          await tx.execute('ALTER TABLE items DROP COLUMN value');
          await tx.execute('UPDATE items SET name = ? WHERE id = ?', [
            'alice!',
            1,
          ]);
        });

        final error = await brokenError.future.timeout(
          const Duration(seconds: 2),
        );
        expect(error, isA<ResqliteQueryException>());

        await healthyUpdated.future.timeout(const Duration(seconds: 2));
        expect(healthyRows.last.single['name'], 'alice!');

        await brokenSub.cancel();
