...
void main() {
  group('SQLite Diagnostics benchmark', () {
    test('runs end-to-end and emits the expected subsections', () async {
      final markdown = await runSqliteDiagnosticsBenchmark();

      expect(markdown, contains('## SQLite Diagnostics'));
      expect(
        markdown,
        contains('### Warm read working set (20000 rows + 2000 point lookups)'),
      );
      expect(
        markdown,
      ...
      expect(
        markdown,
        contains('| Library | SQLite total (KiB) | Page cache (KiB) | '
