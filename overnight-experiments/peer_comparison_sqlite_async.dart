...
/// sqlite_async subprocess for peer comparison benchmark.
...
Future<void> main() async {
  final tempDir = await Directory.systemTemp.createTemp('bench_peer_async_');
  ...
  try {
    ...
    for (var i = 0; i < _warmupLookups; i++) {
      await db.getAll(sql, [(i % _seedRows) + 1]);
    }
    warmupSw.stop();
    ...
    for (var i = 0; i < _lookups; i++) {
      ...
      final rows = await db.getAll(sql, [(i % _seedRows) + 1]);
      sw.stop();
