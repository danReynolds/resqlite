...
///
/// The writer WAL hook only marks a coalesced request. After a successful
/// commit the writer isolate claims that request and sends [CheckpointRun]
/// directly here, keeping checkpoint I/O off the writer's reply path.
...
void checkpointEntrypoint(List<Object> args) {
  ...
  void runClaimedCheckpoints() {
    while (true) {
      ...
      // error is retained for internal diagnostics; the next threshold
      // crossing is still allowed to retry.
      if (rc != 0 && rc != 5) {
  ...
  receivePort.handler = (Object? message) {
    switch (message) {
      ...
      case _CheckpointClose(:final replyPort):
        ...
        // Run unconditionally to drain an already-claimed request even when
        // close overtakes its wakeup. An idle PASSIVE checkpoint is harmless.
        runClaimedCheckpoints();
        receivePort.close();
