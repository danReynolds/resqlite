...
|---|---|---|
| [133](133-multi-row-insert-chunking.md) | Multi-row INSERT chunking | Real but conditional step-count win: 21-pass public guard medians improved narrow 4.48 → 3.22 ms and blob-heavy 12.33 → 10.93 ms, but wide ASCII only moved 18.11 → 17.30 ms. The production implementation required a SQL recognizer, multi-segment transaction planning, profile aggregation, and specialized chunked param packing. Rejected as too much writer/encoder complexity for the breadth of win; implementation archived at `archive/exp-133` for future large simple INSERT workload evidence |
| [128](128-batched-stream-requeries.md) | Batched stream re-queries | Real but narrow A11c-overlap win: Many-Streams overlap improved 65.90 → 24.57 ms and reader replies collapsed, but the value depends on many overlapping streams already being dirtied by the same table writes. It adds reader-worker batch protocol/scheduling complexity and does not solve keyed-PK/table-level miss precision. Implementation rolled back and archived at `archive/exp-128` for future workloads with direct reply-fanout evidence |
