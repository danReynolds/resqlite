...
| [124](124-stream-completion-counter.md) | Stream completion scheduling cost counter | Measurement-only: `streamCompletionUs` / `streamCompletionCount` for the post-await body of `StreamEngine._requery` and `_createStream`. A11c overlap spends ~25-34% of writer-side burst wall in completion-side scheduling vs ~13-17% in invalidation traversal (exp 121); keyed-PK is ~21-32% vs ~13-16%. Disjoint stays at zero (column elision from exp 106). Pairs with exp 123 (writer-isolate vs native) to give end-to-end overlap-burst wall accounting; unblocks the remaining `stream-rerun-dispatch` measurement gap |  |
