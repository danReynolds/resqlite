...
| [150](150-writer-request-residual-split.md) | Writer request residual split | Measurement-only: profile counters split exp 147's residual into request round trip, writer handler, dirty harvest, transfer/resolution, invalidation, and coordination. Dirty harvest is small (A11c overlap 4.25-4.70%, keyed-PK 2.12-2.83%); transfer/resolution and coordination are the larger remaining buckets, so future stream work should not start with SQLite-step or dirty-harvest tuning |  |
