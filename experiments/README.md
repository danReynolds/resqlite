...
| [127](127-writer-dispatch-wall-audit.md) | Writer-isolate dispatch wall audit | Measurement-only: profile-mode `WriterProfileCounters` split writer-isolate handler wall from the inner `resqlite_execute` FFI step. On A11c overlap, writer-isolate work is only 22–24% of total burst wall (Dart-side dispatch 12–14%, SQLite step 9–11%); ~76% of wall sits in main-isolate completion / mutex / IPC. Removes the `writer-isolate wall vs SQLite wall split` `blockedOnMeasurement` entry and steers the next dispatch experiment toward completion-side scheduling, the remaining open candidate |  |
