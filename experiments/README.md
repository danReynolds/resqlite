...
| [134](134-keyed-pk-dirty-elision.md) | Keyed PK dirty rowid elision | Writer-side dirty rowids let verified simple `WHERE id = ?` streams skip miss-path re-queries; keyed-PK profile wall improves 25.54 → 12.45 ms and per-stream intersection probes collapse 10,000 → 3 while many-streams guardrails stay neutral |  |
