...
| [135](135-writer-step-wall-audit.md) | Writer-isolate dispatch wall vs SQLite step wall audit | Measurement-only: profile-mode counters split writer-handler wall into SQLite step (`writerSqliteUs`) and Dart dispatch (`writerHandlerUs - writerSqliteUs`), exposed cross-isolate via `Database.snapshotWriterProfileCounters()`. On A11c overlap the writer handler is only ~22-25% of burst wall (rest is main-isolate microtask scheduling); within the writer, SQLite is ~55-58% and Dart dispatch is ~42-45% — structural ceiling for "remove all writer dispatch on overlap" is ~9-11% of wall, at the per-benchmark decision threshold edge. Closes the `writer-isolate wall vs SQLite wall split` candidate in `signals.json#stream-rerun-dispatch.blockedOnMeasurement`; only completion-side scheduling counter remains |  |
| [136](136-stream-completion-counter.md) | Stream completion-side wall audit | Measurement-only: profile-mode counters (`streamCompleteUs`, `streamCompleteCount`, `streamEmitUs`, `streamEmitCount`) capture the synchronous main-isolate wall inside `StreamEngine._requery` post-`await` body and `StreamEntry.emit`. On A11c overlap, completion accounts for ~30% of writer-burst wall across 4 passes; combined with exp 121's `invalidate_us` (~15%), the synchronous main-isolate stream-engine code accounts for ~45% of wall — leaving ~55% in framework microtask scheduling and async boundaries the counters can't reach. Disjoint stays 0% (column elision filters at invalidation time). Closes the last `completion-side microtask scheduling cost counter` entry in `signals.json#stream-rerun-dispatch.blockedOnMeasurement`; the next dispatch implementation experiment must clear a per-handler `complete_us` target (~7 µs / handler × ~3,750 handlers per overlap burst) |  |
