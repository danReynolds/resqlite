...
|---|---|---|
| [154](154-long-payload-stream-hash.md) | Long-payload stream hash workload | Added the missing mixed 32KB TEXT/BLOB unchanged-fanout workload, then used it to reject a guarded 32-byte unrolled `fnv_combine_bytes` candidate. Target large-payload median moved only 6.014 -> 5.866 ms (-2.5%) while short unchanged fanout and the 4KB long-text guardrail moved worse in the same pass. No runtime code kept; the benchmark remains so future long-payload hash variants can be tested directly. |
