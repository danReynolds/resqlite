...
| [136](136-completion-microtask-counter.md) | Completion-side reader-handler counter | Measurement-only: profile-mode counters (`completion_handler_us`, `stream_emit_us`) expose the main-isolate reader worker port handler chain on A11c overlap as 22–27% of total wall (burst + drain) at ~12 µs per call across ~3,800 calls/burst, with subscriber emit < 1% of the chain and 99% of replies short-circuited by `selectIfChanged` hash. Reader-reply batching is the natural bounded implementation candidate. Closes the `completion-side microtask scheduling cost counter` entry in `signals.json#stream-rerun-dispatch.blockedOnMeasurement` |  |
...
| [147](147-writer-sqlite-wall-split.md) | Writer SQLite wall split | Measurement-only: profile-mode writer responses now report SQLite-facing write time back into `ProfileCounters`; on A11c overlap, SQLite is 26.5 ms / 223.2 ms (11.9%), invalidation is 18.7%, and residual writer/request wall is 69.5%. Keyed-PK shows the same shape (16.9% SQLite, 18.6% invalidation, 64.5% residual). Clears the writer-wall-vs-SQLite-wall blocker; next stream work should measure completion/reply scheduling rather than SQLite-step tuning |  |
