...
| [123](123-writer-dispatch-step-split.md) | Writer dispatch / native wall split | Measurement-only: writer-isolate `_handleExecute` / `_handleBatch` wall now splits into native FFI (`writer_native_us`) and Dart-side dispatch (`writer_handler_us - writer_native_us`) via a `Database.writerProfileSnapshot()` cross-isolate snapshot. Closes the writer-wall measurement gap on both `stream-rerun-dispatch` and `parameter-encoding-and-binding`. Headlines: single Execute path is 65–82 % native (writer dispatch is small); wide batch is only 33–43 % native (Dart batch encoding dominates, 24+ ms recoverable); A11c overlap writer wall is only 34–42 % of total wall (most overlap-fanout cost is outside the writer) |  |
