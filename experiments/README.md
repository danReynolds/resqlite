...
| [123](123-writer-wall-split-audit.md) | Writer wall split audit | Measurement-only: profile-mode writer counters split main-isolate writer roundtrip, worker write-call wall, dirty-dependency fetch, and invalidation. Dirty fetch is <1% of roundtrip after warmup; A11c overlap is residual/invalidation-shaped (write call only 27-32% of roundtrip), while wide batch remains write-call dominated (~83% of roundtrip). Exact SQLite statement timing is deferred because this build uses `SQLITE_OMIT_TRACE` |  |
