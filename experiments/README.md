...
| [139](139-sustained-park-stress.md) | Sustained concurrent-reads parking stress | Measurement-only: profile harness sustains pool-overflow concurrency for 1s per pass. `wake_retry_total = 0` at concurrency 4/8/16/32 across 80k–97k parks per pass; `max_parked = concurrency - pool_size` exactly. Confirms exp 118's FIFO holds under continuously-refilled waiter queues and out-of-order slot release. Closes the `long-running concurrent-reads workload that sustains parked dispatchers past pool size` open candidate |  |
