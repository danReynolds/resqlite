...
| [149](149-residual-writer-wall-split.md) | Residual writer/request wall split | Measurement-only: subdivides exp 147's residual via three new counters (`writer_handle_us`, `writer_dirty_us`, `main_writer_reply_us`). On A11c overlap and keyed-PK, writer dirty-set harvest is 2–10% of wall, writer reply send is 0.5–1.4%, main-isolate writer reply resolution is 1.0–2.6% — each individually below the Tracelite primary gate. Dominant bucket is `rest_us` (46–67% of wall); on overlap workloads two-thirds of `rest_us` is the same exp-136 reader-completion handler chain exp 148 already targeted. Removes exp 148's four-bucket "dirty/send/main-resolution/drain coordination" framing from active candidates |  |
