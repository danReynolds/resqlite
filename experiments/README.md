...
| [117](117-named-parameters.md) | Named parameter support | Adds `Map<String, Object?>` named binds (`:name`/`@name`/`$name`/`?NNN`) on every public parameter-taking method while keeping the positional hot path within noise; suite-level 16 wins / 2 regressions / 143 neutral with focused 5-run on wide batch insert at +3% (overlapping with main's noise range) |  |
