...
| [138](138-blob-param-shape-audit.md) | Blob-heavy batch parameter shape audit | Measurement-only: extends `batch_param_flatten.dart` with `--cell-mode=blob --blob-size=N` and sweeps 4 / 64 / 256 / 1024 / 4096 byte blobs across 100/1000/10000 × 2/8/20 shapes. Per-byte throughput converges to a 137–298 MB/s SQLite-stepping band that matches exp 125 / 126's wide-text shapes, confirming the blob path has no removable Dart-side encoding cost (the encoder writes the caller's `Uint8List` directly into the inline parameter buffer via `view.setRange(...)`). Closes the `parameter-encoding-and-binding` blob-encoding openQuestion |  |
