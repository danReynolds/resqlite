...
| [142](142-single-row-text-direct-encoding.md) | Single-row text parameter direct encoding | Removes the per-call `List<Uint8List?>` and per-string `utf8.encode` allocations from `allocateParams`; focused single-row INSERT shapes improve −13 % to −26 % on ASCII (Unicode 3-string row −18 %, single Unicode strings flat — dual-walk balances against `utf8.encode` + memcpy until allocations stack up), release Single Inserts row improves 5.423 → 4.327 ms (−20.2 %), batch/wide-batch rows route through unchanged encoders |  |
