...
|---|---|---|
| [155](155-stream-rerun-version-coalesce.md) | Stream rerun version coalescing | A dirty-while-in-flight token tried delaying only follow-up stream re-runs to the next event turn, but the profile smoke did not reduce A11c overlap completion churn (`completion_count` 4,318 -> 4,405) and keyed-PK total wall moved the wrong way. A wider 1 ms window reduced some callbacks but made wall much worse. The formal Tracelite stream-dispatch A/B did not clear any primary gate: high-cardinality fanout -0.52%, keyed-PK -0.88% but too noisy, many-streams writer throughput +4.32%. No runtime code kept. |
