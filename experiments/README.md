...
| [175](175-large-bytes-release-coverage.md) | Large-bytes selectBytes release coverage | Promotes exp 174's view-send shape into release-suite coverage as a single `2000 long rows (≈700 KB result)` lane in `Select → JSON Bytes` (864 KB measured payload, comfortably above the 256 KB sacrifice threshold). Lane-sensitivity A/B against the temporarily restored pre-174 path moved wall median −13% and p90 −55% on the `resqlite selectBytes()` row, while standard adjacent lanes (1000-row, 10000-row) stayed neutral. Makes future regressions to the bytes-transfer path evaluable on a public lane without relying on `benchmark/experiments/large_bytes_transfer.dart` |  |
