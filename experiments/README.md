...
| [127](127-writer-isolate-wall-split.md) | Writer-isolate wall vs SQLite step wall split | Measurement-only: cross-isolate writer counters (`handler_us`, `sqlite_us`, `handler_count`) pair with the existing `audit_workloads.dart` scenarios. On A11c overlap the writer is busy 24–26% of burst wall (SQLite 13–15%, Dart 11–12%); the remaining ~75% is wall the writer is idle between requests, which maps to the still-blocked completion-side scheduling cost counter. Resolves the writer-side `blockedOnMeasurement` entry on `stream-rerun-dispatch` and `measurement-system` |  |
