...
| [094](094-skip-column-count-ffi-on-cache-hit.md) | Skip `sqlite3_column_count` FFI on schema-cache hit | Savings ceiling is one leaf FFI call (~25 ns) per decoded query — 0.2 % of an 11 µs point query and 0.0003 % of an 8.8 ms 10k-row decode. Single-run release-mode A/B swung ±10–98 % on workloads that can't be affected by the change (e.g. `selectBytes` doesn't touch `decodeQuery`, point-query throughput regressed 12 %). Noise dwarfs signal. Same class as exp 076 and 093 |
