...
| [168](168-uncontended-write-mutex-fast-path.md) | Synchronous fast path for uncontended writer mutex | **−4% to −9%** on focused `writer_pipelining` `sequential-awaited` (2000 writes) across three paired passes by skipping the microtask hop `await _mutex.lock()` always pays — even uncontended — through a new `Mutex.tryLock()` and non-async `Writer.execute` / `Writer.executeBatch`. Concurrent burst improves **−2.6% to −4.8%**; `transaction-guardrail` (untouched `Writer.locked` path) stays neutral. |  |
