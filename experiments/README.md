...
| [162](162-concurrent-reads-pool-scaling.md) | Sustained concurrent reads + pool-cap re-test | Paired run under the new runner rule: ships the reads-saturation lane the signal map requested since April (counter-gated: 12,795/12,800 requests park at cap 4) **and** re-tests exp 105's rejected cap raise against it — reads gain +10–38% but A11c overlap regresses +36% and keyed-PK +29% on the current (post-118/120/122) dispatch structure, so the rejection is re-confirmed as current and the candidate is closed |  |
