# Experiment 150: Writer request pipelining + persistent reply port
...
counters without moving measured elapsed. The in-flight exp 149
(`exp-149-residual-writer-wall-split`) subdivided the residual further and
concluded that no single writer-side sub-bucket clears the decision
threshold — "future stream-dispatch work needs a workload-shape or
scheduling-model change rather than another sub-bucket optimization."
...
**Pass 2 (order flipped: exp-150 code collected first, main second)**
...
- Concurrent writes — the workload shape the in-flight exp 149 split
  pointed at — improve 36–45% on the focused benchmark.
