...
**Benchmark Run:** none — focused A/B
  [`benchmark/experiments/slot_sacrifice_ab.dart`](../benchmark/experiments/slot_sacrifice_ab.dart);
  raw table in
...
sites. It routes on `raw.values.length > sacrificeSlotThreshold` (default
`32 * 1024`). The legacy byte path is retained behind a compile-time toggle
(`RESQLITE_SLOT_TRIGGER`, default the slot trigger) as a soak-window rollback and
to reproduce the A/B; it can be removed once the slot trigger has soaked.
`selectBytes` still never sacrifices (unchanged). A benchmark-only
`ReaderPool.debugSacrificeCount` proves the routing actually flips.
...
threshold had it. The byte path stays as a compile-time rollback for the soak.
...
sacrifice. Under slot-count routing that subtraction becomes moot — blob *size*
no longer influences the decision at all — so the two compose cleanly and the
byte machinery (`estimatedBytes`, the decoder's per-cell `byteEstimate`) can be
retired together once both have soaked.
