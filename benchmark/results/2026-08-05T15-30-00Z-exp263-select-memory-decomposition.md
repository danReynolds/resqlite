...
seeded range. (An earlier revision reported 137.8 B, averaged over the first 100
rows; `Item $i` and the description's `$i` both grow with the row index, so a
...
varint-encodes integers and carries a per-row header.
