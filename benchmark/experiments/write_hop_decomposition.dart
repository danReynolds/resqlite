...
//
//           The in-situ probes that produced the per-slice table in the writeup
//           lived in `lib/` for the length of the run and are preserved at
//           `archive/exp-284`; they are not part of the retained harness,
//           because the four lanes below answer the question without them.
//
...
//
// Every number in the writeup is AOT. `dart compile exe` cannot resolve the
// native asset, so the entry point is copied into `bin/` (untracked) and built
// with `dart build cli`:
//
...
/// One monotonic clock for every lane that times individual calls.
