...
Workload: `SELECT v FROM items WHERE v >= ? AND v < ?` fanned out at the listed concurrency. Each burst awaits all queries; counters are reset between bursts and the median across bursts is reported.
...
- `parked_total` increments each time `_dispatch` awaits `_workerAvailable` after finding every slot busy.
